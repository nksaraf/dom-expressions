      ...
      if (child.text) {
        ...
        if (multi) {
          results.decl.push(
            t.variableDeclarator(
              child.id,
              t.callExpression(createTextNode, [t.stringLiteral(decode(child.template))])
            )
          );
        } else insert = t.callExpression(createTextNode, [t.stringLiteral(decode(child.template))]);
      }
